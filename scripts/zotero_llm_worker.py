...
def route_attachment(attachment: Attachment, path: Path, page_count: int, config: Config) -> tuple[str, str, int]:
...
def attachment_tags(attachment: Attachment, route: str) -> list[str]:
    tags = [f"ocr-source:{attachment.key}", "zotero-llm", "markdown"]
    if route == "paddle-ocr":
        tags.extend(["ocr:paddleocr-vl-1.5", "ocr-md"])
    else:
        tags.extend(["pdf-text", "born-digital"])
    return tags
...
def process_text_route(
    *,
    ...
    zotero_key = zotero.create_markdown_attachment(
        ...
        tags=attachment_tags(attachment, "pdf-text"),
    )
...
def process_ocr_route(
    *,
    ...
    zotero_key = zotero.create_markdown_attachment(
        ...
        markdown_path=markdown_path,
        tags=attachment_tags(attachment, "paddle-ocr"),
    )
    if replace_attachment_key and replace_attachment_key != zotero_key:
        zotero.delete_item(replace_attachment_key)
    status = selection_status(pages, page_count, uploaded=True)
...
def process_attachment(
    *,
    ...
    if force_route:
        ...
        route, route_reason, sampled_chars = route_attachment(attachment, attachment.path, page_count, config)
    logging.info(
...
def upload_test(config: Config, state: StateDB, local: ZoteroLocalClient, key: str) -> None:
    ...
    zotero_key = ZoteroWebClient(config).create_markdown_attachment(
        ...
        markdown_path=markdown_path,
        tags=attachment_tags(attachment, route),
    )
    source_md5 = md5_file(attachment.path)
    page_count = pdf_page_count(attachment.path)
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.force_ocr and args.force_text:
        raise WorkerError("Use only one of --force-ocr or --force-text")
    ...
    force_route = "paddle-ocr" if args.force_ocr else "pdf-text" if args.force_text else None
