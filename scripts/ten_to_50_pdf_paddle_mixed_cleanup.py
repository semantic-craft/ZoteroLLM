...
"""Convert current 10-50MB Zotero PDFs to PaddleOCR-VL Markdown.
...
MIN_SIZE_MB = 10.0
MAX_SIZE_MB = 50.0
REPORT_PATH = APP_ROOT / "reports" / "ten_to_50_pdf_paddle_mixed_cleanup.md"
TABLE_NAME = "ten_to_50_pdf_cleanup_jobs"
...
def write_report(conn: sqlite3.Connection) -> None:
    ...
    lines = [
        "# 10-50MB PDF PaddleOCR Mixed Cleanup",
        "",
        f"- Updated: `{dt.datetime.now().astimezone().isoformat(timespec='seconds')}`",
        "- Scope: current Zotero PDF attachments with local size `>10MB` and `<=50MB` at enumeration time.",
...
def process_targets(args: argparse.Namespace) -> int:
    ...
    logging.info("Seeded %s live 10-50MB PDF targets", seeded)
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="PaddleOCR current 10-50MB PDFs, keeping clear-law sources and deleting adjacent/non-law sources."
    )
