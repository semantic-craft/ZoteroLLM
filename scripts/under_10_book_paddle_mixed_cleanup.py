...
"""Convert current <10MB Zotero book PDFs to PaddleOCR-VL Markdown.
...
from ten_to_50_pdf_paddle_inventory import (  # noqa: E402
    classify as inventory_classify,
    read_parent_data,
)


MAX_SIZE_MB = 10.0
REPORT_PATH = APP_ROOT / "reports" / "under_10_book_pdf_paddle_mixed_cleanup.md"
TABLE_NAME = "under_10_book_pdf_cleanup_jobs"
...
def classify_target(local: ZoteroLocalClient, attachment: Attachment) -> str:
    parent_data = read_parent_data(local, attachment.parent_key)
    classification, _basis = inventory_classify(attachment, parent_data)
...
def seed_live_targets(conn: sqlite3.Connection, local: ZoteroLocalClient) -> int:
    ...
    for attachment in local.iter_pdf_attachments():
        ...
        if not (0 < size_mb < MAX_SIZE_MB):
            continue
        title = attachment.parent_title or attachment.title or "未命名"
        classification = classify_target(local, attachment)
        upsert_job(
            conn,
...
def write_report(conn: sqlite3.Connection) -> None:
    ...
    lines = [
        "# Under 10MB Book PDF PaddleOCR Mixed Cleanup",
        "",
        f"- Updated: `{dt.datetime.now().astimezone().isoformat(timespec='seconds')}`",
        "- Scope: current Zotero `book` PDF attachments with local size `<10MB` at enumeration time.",
...
def is_zotero_local_api_unavailable(exc: Exception) -> bool:
    message = str(exc)
    markers = (
        "Connection refused",
...
def process_targets(args: argparse.Namespace) -> int:
    ...
    logging.info("Seeded %s live under-10MB book PDF targets", seeded)
    ...
    for target in load_targets(state.conn):
        ...
        try:
            ...
            if row:
                ...
                continue
            if is_zotero_local_api_unavailable(exc):
                upsert_job(
                    state.conn,
                    pdf_key=target.pdf_key,
                    title=target.title,
                    ...
                    size_mb=target.size_mb,
                    md_status="stopped",
                    error=f"Zotero local API unavailable while reading source attachment: {exc}",
                )
                logging.error("Stopping at %s because Zotero local API is unavailable: %s", target.pdf_key, exc)
                break
            upsert_job(
                state.conn,
                pdf_key=target.pdf_key,
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="PaddleOCR current under-10MB book PDFs, keeping clear-law sources and deleting adjacent/non-law sources."
    )
