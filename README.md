...
- Markdown attachment filenames follow `作者_年份_题名.md`, for example `张三_2025_示例文献标题.md`.
...
- `.env`: local secrets/config, chmod 600.
...
```bash
/opt/homebrew/bin/python3.11 scripts/zotero_llm_worker.py --attachment-key EXAMPLEKEY --pages 1-1 --no-upload --force-text
```

Upload the latest generated Markdown for one PDF:

```bash
/opt/homebrew/bin/python3.11 scripts/zotero_llm_worker.py --upload-test --attachment-key EXAMPLEKEY
```
